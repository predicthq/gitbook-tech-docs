...
Under the hood, location-based access uses latitude, longitude, and radius to control what you can access. In the UI you can enter a street address for a business location and the product will find the latitude and longitude for that location and set a [predicted impact area](https://app.gitbook.com/s/kEFs8urDbSJqBmXUI3Lv/impact-area/get-impact-area) and save the location with those details.
...
2. Call the Create Saved Locations endpoint (POST /saved-locations) using `origin_geojson` with the lat/lon for each location — Predicted Impact Area is calculated automatically and stored as the location boundary, so no separate API call is needed
