...
* **Grounding (MCP server + Events API)** - verified context retrieved by AI systems at answer time, from a store inside your environment (provisioned grounding) or on demand via MCP (on-demand grounding)
