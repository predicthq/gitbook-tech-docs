...
{% updates format="full" %}
{% update date="2025-12-17" tags="data-quality,enhancement,events-api" %}
## Premier League Titles - Consistent League Prefix

Premier League soccer event titles now consistently carry the `Premier League - ` prefix. Customers filtering or matching on the league prefix can rely on the same title format across the season.
{% endupdate %}

{% update date="2025-12-12" tags="data-quality,enhancement,events-api" %}
## Rescheduled Event Details Moved to the Description

For rescheduled events, the "Rescheduled from" detail now appears in the event description rather than the event title. Titles stay consistent, and the rescheduling detail remains available on the event.
{% endupdate %}

{% update date="2025-12-10" tags="data-quality,enhancement,events-api" %}
## Sports Event Titles - Geography-Based Team Ordering

Sports event titles now order the competing teams according to the convention used in the country where the fixture is played. US and Canadian fixtures use _Away vs Home_, with soccer as the exception at _Home vs Away_. Everywhere else - Europe, Latin America, Asia, Africa, and Oceania - uses _Home vs Away_, and international competitions such as FIFA tournaments and the Olympics always use _Home vs Away_, including at neutral venues.
{% endupdate %}
...
{% update date="2025-11-24" tags="data-quality,enhancement" %}
## Predicted End Time - Major Performing Arts

Predicted End Time predictions have been improved for major performing arts events.
{% endupdate %}

{% update date="2025-11-10" tags="data-quality,enhancement" %}
## Handball Predicted Attendance Model

A predicted attendance model for handball is now live, following the addition of handball coverage earlier in the year.
{% endupdate %}

{% update date="2025-10-21" tags="enhancement,webapp" %}
## Current Location in Address Search

Address forms in the WebApp now include a crosshair control that fills in your current location from the browser. Your location is only requested when you click the control.
{% endupdate %}
...
{% update date="2025-09-24" tags="enhancement,webapp,features-api" %}
## Local Rank Filter in Event Trends

Event Trends now filters on Local Rank, bringing it into line with the rank filter on the events page. Local Rank is supported in the Features API and is the recommended rank to filter on.
{% endupdate %}

{% update date="2025-09-24" tags="enhancement,webapp,saved-locations" %}
## Location ID Visible in Saved Locations

The `location_id` for a Saved Location is now shown alongside the location name on the location details page, so you no longer need to take it from the browser URL.
{% endupdate %}
...
{% update date="2025-09-01" tags="webapp,deprecation" %}
## Notifications Service Retired

The Notifications service was retired on 31 August 2025. From 28 July 2025, new notifications could no longer be created, while existing notifications remained viewable and could be deleted up until the retirement date.
{% endupdate %}
...
{% update date="2025-08-19" tags="new-feature,data-delivery" %}
## Entities Field in Data Exports

Event exports now include an `entities` column containing all entities on a given event, for the entity types your subscription includes. Previously only the venue entity was available, unrolled into individual columns in a CSV export. Event group entities and other entity types are now available directly from the export. Venue entity fields for broadcasts have also been updated to use the venue-type-specific entity.
{% endupdate %}

{% update date="2025-08-05" tags="enhancement,beam,webapp" %}
## Beam - Holidays and Observances on the Graph

School holidays, public holidays, and observances are now shown on the Beam Analysis graph as a shaded background behind the demand plots. This makes holiday-driven demand movements easier to read alongside attended events - particularly useful for retail customers, who are heavily impacted by these event types.
{% endupdate %}

{% update date="2025-07-28" tags="data-quality,enhancement" %}
## Predicted End Time - Major Concerts

Predicted End Time predictions have been improved for major concerts, based on research into when large shows actually finish. High-attendance concerts starting in the late afternoon and early evening are now handled separately.
{% endupdate %}

{% update date="2025-07-21" tags="data-quality,enhancement,events-api" %}
## Sports Event Durations and Deduplication

Two improvements to sports event data quality. Single-day fixtures now consistently carry a single-day duration, with a rule in place to keep differing end dates in source data from extending a fixture across multiple days. Deduplication has also been improved so that records for the same fixture whose times differ by an hour are matched into a single event.

Together these give a cleaner sports catalogue, with one record per fixture and accurate impact windows.
{% endupdate %}

{% update date="2025-07-15" tags="enhancement,webapp,events-api" %}
## Active and Impact Date Filtering in the WebApp

Event Search, Saved Locations, and Event Trends now let you switch between `active` and `impact` date parameters directly in the date range filter. `impact` uses demand impact patterns to include leading and lagging days around an event, while `active` restricts results to the event days themselves.

This brings the WebApp in line with the Events API, and makes it explicit which date behaviour is driving the result set you are looking at.
{% endupdate %}
...
{% update date="2025-07-05" tags="data-quality,enhancement" %}
## US School Holidays - 2025–2026 Calendar Year
...
{% update date="2025-05-28" tags="new-feature,data-delivery" %}
## Total Predicted Event Spend in Snowflake and ADX

A total Predicted Event Spend field is now available in Snowflake and ADX, alongside the existing breakdown of PES fields. Customers whose subscription includes total PES but not the breakdown can now access the total directly. Subscription requirements apply to the new field in the same way as the existing breakdown, and the data dictionaries in the tech docs and on the ADX listings have been updated.
{% endupdate %}

{% update date="2025-05-27" tags="data-quality,enhancement,broadcasts" %}
## Live TV Viewership - UFC, Major Boxing, and WrestleMania

Viewership for future UFC events, the most anticipated major boxing matches, and WrestleMania has been added for the United States, and these are now available as events with viewership. This came from customers using Live TV events in their models, for whom these events have a major impact on forecasts.
{% endupdate %}

{% update date="2025-05-08" tags="enhancement,beam,webapp" %}
## Beam Charts - Y-Axis Zoom and Scaling

You can now adjust the y-axis range on Beam Analysis charts. This makes it easier to see smaller events alongside the largest ones, particularly for attendance, where the scale can run into the millions.
{% endupdate %}

{% update date="2025-04-30" tags="data-quality,enhancement" %}
## UK School Holidays - District-Level Records

PredictHQ school holiday records can now be created at GSS district level for the United Kingdom, extending district-level record creation that was previously available for the US. This supports predicted school holiday coverage for the UK.
{% endupdate %}

{% update date="2025-04-30" tags="data-quality,enhancement" %}
## Handball Event Coverage

Handball is now covered across all leagues, including Germany's Handball-Bundesliga, which features some of the best handball teams and players in the world, alongside other highly competitive leagues. The sport has been growing in popularity across European countries.
{% endupdate %}

{% update date="2025-04-28" tags="new-feature,data-quality" %}
## Predicted School Holidays
...
{% update date="2025-04-06" tags="data-quality,enhancement" %}
## US Academic Events - 2025–2026 Calendar Year
