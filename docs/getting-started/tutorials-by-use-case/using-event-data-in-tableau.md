...
1. Access the WebApp: Log in and navigate to [Search Events](https://control.predicthq.com/search/events).&#x20;
...
{% hint style="info" %}
For guidance on finding the most relevant events for your business, see [filtering-and-finding-relevant-events.md](filtering-and-finding-relevant-events.md "mention"). For searches around specific locations or stores, exporting a JSON file from the WebApp [Saved Locations](https://control.predicthq.com/location-insights) is recommended.
{% endhint %}

**Export as JSON**

3. Export Events: Once the events of interest are displayed on the WebApp Search, click 'Export' on the right-hand side followed by 'Export Events Data'. In the dialog box that appears, select the 'JSONL' tab and then click 'Export'.&#x20;
...
{% hint style="info" %}
For more information on using the WebApp Search and exporting files, see [export-data-via-control-center.md](../guides/export-data-via-control-center.md "mention").
{% endhint %}
